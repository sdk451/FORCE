# Agentic assembly — forge-vibe-code-enhancement

Use this checklist **after** `vibeforge` has written baseline files (`AGENTS.md`, host rules, optional skills). Root **`AGENTS.md`** at that point is the **canonical scaffold** (structure + placeholders), not the finished tuned file. Goal: turn that scaffold plus your notes into **project-specific** instructions without starting from a blank page.

**Preview without writing files:** run **`vibeforge blueprint`** (`--yes`, `--answers <file>`, or interactive) to print a JSON bundle you can paste into a coding agent.

**Invoke a host CLI:** run **`vibeforge assemble`** (see **`docs/FORGE-ASSEMBLE.md`**) to create a **temp assembly workspace** (prompt + copies under OS temp), then spawn **Claude**, **Cursor** (`cursor agent`), **GitHub Copilot CLI** (`copilot`), **Gemini**, or **Codex** when installed. The CLI **deletes** **`forge_vibe_agent_instructions_done.txt`** at the repo root before each invoke (if present). On **CLI exit 0**, success requires **`forge_vibe_agent_instructions_done.txt`** at the project root **or** meaningful **AGENTS.md** progress (rewritten off the install scaffold). Fails (**exit 1**, workspace kept) only if **neither** is true. Idempotent second run: **AGENTS.md** already tuned and unchanged still exits **0** without a new marker. With **`--no-invoke`**, no CLI on `PATH`, or other **failed** runs, stderr/stdout includes the IDE block with the **temp path** and cleanup instructions.
