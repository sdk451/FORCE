...
**Invoke a host CLI:** run **`forge-vibe assemble`** (see **`docs/FORGE-ASSEMBLE.md`**) to create a **temp assembly workspace** (prompt + copies under OS temp), then spawn **Claude**, **Cursor** (`cursor agent`), **GitHub Copilot CLI** (`copilot`), **Gemini**, or **Codex** when installed. On **CLI exit 0**, **forge-vibe** checks root **AGENTS.md** (normalized line endings): it fails (**exit 1**, workspace kept) only if the file is **still the exact install template** *and* **unchanged since the invoke** (nothing saved). If **AGENTS.md** already differed from that template before the run, a second assemble with **no on-disk edits** exits **0** (idempotent). If the file **changed** off the template this run, exit **0** and the temp folder is removed. With **`--no-invoke`**, no CLI on `PATH`, or other **failed** runs, stderr/stdout includes the IDE block with the **temp path** and cleanup instructions.
