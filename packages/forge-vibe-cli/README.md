...
- **`assemble`** — After a write/install: reads the install profile, writes **`docs/FORGE-ASSEMBLE-PROMPT.md`**, then tries **Claude**, **Cursor** (`cursor agent`), **GitHub Copilot CLI** (`copilot`), **Gemini**, or **Codex** when on `PATH` and enabled in the profile (`--agent auto`). **`--no-invoke`** or missing CLIs prints a **copy-paste IDE chat** on stdout (absolute path to the prompt) for **Cline**, **Kimi**, IDE Copilot chat, etc. Requires vendor CLI auth/network when spawning.
