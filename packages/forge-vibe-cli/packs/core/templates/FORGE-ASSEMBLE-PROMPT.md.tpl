...
4. **Optional skills & packs:** if the profile lists **`optional_skills`** or UI/memory/hooks flags, keep **only** end-user content in **`AGENTS.md`**: each skill/pack **display name** and **when to use it** (repo-specific triggers). **Do not** paste the **assembly reference** block from this prompt into **`AGENTS.md`**. Host paths, **`forge-*` ids**, **`SKILL.md`**, and **docs/FORGE-COMPATIBILITY-MATRIX.md** belong in **CLAUDE.md** / **GEMINI.md** / rules — not in the portable runbook.
...
Match `docs/FORGE-AGENTIC-ASSEMBLY.md`, and confirm **`{{AGENTS_MD_ABS}}`** on disk reads like a **project runbook** (top **15–20** element themes addressed, no menu-style What/Why/Example blocks). If the profile lists **optional skills** or optional packs, confirm **`AGENTS.md`** still names each and **when to use it** — without installer meta, **`forge-*` paths**, or matrix/skill-discovery prose. Re-open the file locally to verify placeholders are gone.
