...
- **Interactive terminal:** while a CLI runs, forge-vibe shows a **spinner** on stderr (TTY only; skipped in CI / non-TTY so logs stay clean).
...
Almost all wall-clock time is **inside the vendor coding agent**, not in forge-vibe: the model reads the assembly prompt, explores the repo (many tool calls), rewrites **AGENTS.md**, then may update **CLAUDE.md**, **.cursor/**, etc. per the BMAD phases. forge-vibe only waits on that process. There is no separate “fast path” that preserves the same edits—shortening the run means changing how the agent works (e.g. vendor CLI settings such as **Claude Code** `--effort`, or a smaller assembly prompt), which trades speed against thoroughness.
