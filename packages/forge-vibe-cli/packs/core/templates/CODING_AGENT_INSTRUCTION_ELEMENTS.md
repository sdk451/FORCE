# Coding Agent Instruction Set Elements: Complete Analysis
**Research Date:** April 2026  
**Sources Analyzed:** 50+ CLAUDE.md, AGENTS.md, GEMINI.md, .cursorrules, .windsurfrules examples  
**Format Coverage:** Claude Code, Cursor, Windsurf, GitHub Copilot, Gemini CLI, OpenCode, Codex

---

## Executive Summary

Modern coding agents (Claude Code, Cursor, GitHub Copilot, Gemini CLI) operate on instruction files—persistent configuration documents that provide context at the start of every session. Analysis of 50+ production instruction sets reveals **60+ distinct element categories** that engineers use to guide agent behavior. These elements cluster into 8 major domains: **Foundation** (project context), **Standards** (code conventions), **Execution** (build/deploy workflows), **Safety** (guardrails), **Architecture** (system design), **Quality** (testing/review), **Knowledge** (reference material), and **Orchestration** (agent coordination).

The most effective instruction sets are **concise, specific, and example-driven**—typically 150–300 lines, leading with rules rather than explanations. Files exceeding 500 lines show degraded agent performance due to instruction-following decay.
...
```
## Project Overview
Symbotic is an agentic AI delivery platform for enterprise software teams. 
It orchestrates Claude Code sessions, tracks technical debt, and auto-generates ADRs.
```
...
- JSDoc example:
   ...
   */
- Avoid: comments restating the code (e.g., "increment i by 1" for i++)
```
...
- Cursor Rules Guide (cursor.com/docs/rules)
- HumanLayer: Writing Good CLAUDE.md
- Vercel: Compressed Context for Agents
- SSW Rules: Agent Skills, MCP, Subagents
- Multiple production CLAUDE.md files analyzed from GitHub (Anthropic/claude-code, hesreallyhim/awesome-claude-code, alirezarezvani/claude-skills, diet103/claude-code-infrastructure-showcase, and 40+ others)
